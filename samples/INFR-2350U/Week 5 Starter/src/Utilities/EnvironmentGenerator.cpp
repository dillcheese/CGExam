	...
	for (int i = 0; i < _objectsToSpawn.size(); i++)
	{
		std::vector<GameObject> temp;
		{
			...
			for (int j = 0; j < _numToSpawn[i]; j++)
			{
				...
				//temp[j].get<Transform>().SetLocalRotation(Util::GetRandomNumberBetween(glm::vec3(0.f, 0.f, 0.f), glm::vec3(0.f, 360.f, 0.0f)));
				temp[j].get<Transform>().SetLocalScale(glm::vec3(2.75f));
				//temp[j].get<Transform>().SetLocalScale(glm::vec3(1.5f * (float)_numToSpawn.size()));
				temp[j].get<Transform>().SetLocalRotation(glm::vec3(90.f, 0.f, 0.0f));
			}
		}
